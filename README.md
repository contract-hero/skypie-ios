...
The iOS companion of [Sky Pie](https://github.com/alilloig/skypie-core): pair
...
```bash
git clone --recurse-submodules git@github.com:alilloig/skypie-ios.git
cd skypie-ios
pnpm install
...
Physical device: `./scripts/install-ios-device.sh`. The RevenueCat public key
comes from the environment (`scripts/inject-revenuecat-key.sh`); without it the
build is a working reader with no purchase.
